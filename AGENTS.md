...
- Do NOT start a dev server or run preview tools for non-UI changes (data files, tests, configs, scripts, .gitignore, AGENTS.md, package.json)
...
### Content Quality
- **Voice skill**: `~/.Codex/skills/sadhaka-voice.md` (v2.0) — THE authority for all prose. Overrides LLM defaults. Integrates stop-slop rules.
- **Anti-slop references**: `~/.Codex/skills/stop-slop/references/` — phrase, structure, and example catalogs
...
- **Publish gates** (all three must pass before commit):
  ...
  - **GEO Citability Score** ≥ 8/10 (body-level extractability — see `.Codex/skills/seo-optimize/SKILL.md` §11.5)
...
5. **Agent module loading**: Load only the relevant agent module (`docs/agents/0X-*.md`) — never load all modules at once. The AGENTS.md index tells you which one to load.
