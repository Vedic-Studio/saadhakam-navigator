...
2. **Read existing learnings**: Browse `.claude/skills/debrief/learnings/` subdirectories to understand patterns already captured. Avoid duplicates.
...
- **Engagement metrics** (any available):
  ...
  - AI citation: whether the article is being cited by AI engines (ChatGPT, Perplexity, Claude)
...
Save each learning to `.claude/skills/debrief/learnings/{skill-name}/` using this format:
...
- **20+ learnings per skill**: suggest consolidation into a summary file (`.claude/skills/debrief/learnings/{skill}/summary.md`)
