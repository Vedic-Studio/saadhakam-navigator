...
| 2026-03-10 | Weekly | Expanded the next concept wave around practice and devotion terms by adding `mantra`, `japa`, `dhyana`, `sadhana`, `om`, `guna`, `avatara`, and `tapas`, then promoted those concept + lexicon URLs into the priority indexing batch | Concept coverage moved deeper into beginner-practice and devotional intent, with stronger alignment to live lexicon entries and LLM-style seeker questions | Submit the refreshed priority batch in production, then continue the next concept wave for ritual and subtle-body terms such as `puja`, `darshan`, `nadi`, and `kundalini` |
