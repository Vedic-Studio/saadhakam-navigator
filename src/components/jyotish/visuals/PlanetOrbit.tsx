...
function OrbitingPlanet({ color, emissive }: { color: string; emissive: string }) {
...
function Scene({
  ...
  return (
    <>
      ...
      <StarField />
      <CentralSun />
      <OrbitingPlanet color={color} emissive={emissive} />
...
export function PlanetOrbit({ name, color, exaltation, debilitation }: PlanetOrbitProps) {
  const grahaKey = name.toLowerCase();
  const palette = GRAHA_COLORS[grahaKey] || { primary: "#f97316", emissive: "#ea580c" };
