/**
 * Faith Finder Email Sequence — Full Copy
...
function wrap(headerTitle, headerSub, body, ctaUrl, ctaText) {
...
// ─── Email 0: Immediate results ─────────────────────────────────────────────────

function email0(result) {
  ...
  const body = `
    ...
    <div class="card">
      ...
      <p>${getPathDescription(result.primaryPath)}</p>
    </div>
...
// ─── Email 1: Day 1 — First practice ────────────────────────────────────────────

function email1(result) {
  const m = PATH_META[result.primaryPath] || PATH_META.inquiry;
...
// ─── Email 2: Day 3 — The tradition ─────────────────────────────────────────────

function email2(result) {
  const m = PATH_META[result.primaryPath] || PATH_META.inquiry;

  const TRADITION_DETAIL = {
    inquiry: {
  ...
  const body = `
    ...
    <div class="card">
      <h3>Teachers Worth Knowing</h3>
      <p>Explore the lives of the great masters who embodied this path on opensadhaka.com — their biographies, their methods, their words.</p>
    </div>
  ...
  return wrap(
    'Day 3: The Tradition',
    `${m.tradition} — what you're walking into`,
    body,
    td.link,
    td.linkText + ' →'
  );
...
// ─── Email 3: Day 7 — One week reflection ───────────────────────────────────────

function email3(result) {
  const m = PATH_META[result.primaryPath] || PATH_META.inquiry;
  ...
  const body = `
    ...
    <div class="quote">
      "A little practice, done regularly and with full attention, is worth more than long sessions done occasionally."
      <cite>— Traditional teaching</cite>
    </div>
...
// ─── Email 4: Day 14 — The philosophy ───────────────────────────────────────────

function email4(result) {
  const m = PATH_META[result.primaryPath] || PATH_META.inquiry;
  ...
  const body = `
    ...
    <div class="card">
      <h3>For deeper study</h3>
      <p>Read the full article on <strong>${m.philosophyArticle}</strong> on opensadhaka.com — written specifically for practitioners at this stage.</p>
    </div>
  ...
  return wrap(
    ...
    `The ideas that make the practice make sense`,
    body,
    m.philosophyLink,
    `Read: ${m.philosophyArticle} →`
  );
}

// ─── Email 5: Day 30 — What comes next ──────────────────────────────────────────

function email5(result) {
  const m = PATH_META[result.primaryPath] || PATH_META.inquiry;
  ...
  const body = `
    ...
    <div class="card">
      ...
      <p>Three concrete next steps, in order of simplicity:</p>
    </div>
...
// ─── Helpers (re-exported for server use) ──────────────────────────────────────

function getPathDescription(pathKey) {
  const descriptions = {
  ...
  return descriptions[pathKey] || '';
}

function getPathTitle(pathKey) {
  const titles = {
    inquiry: 'Path of Inquiry',
    devotion: 'Path of Devotion',
    ritual: 'Path of Ritual',
    discipline: 'Path of Discipline',
  };
  return titles[pathKey] || 'Your Spiritual Path';
}
...
module.exports = {
  getPathDescription,
  getPathTitle,
  getSubject: (emailIndex, pathKey) => SUBJECTS[emailIndex](pathKey),
  buildEmail: (emailIndex, result) => {
    const fns = [email0, email1, email2, email3, email4, email5];
    const fn = fns[emailIndex];
    if (!fn) return null;
    return fn(result);
  },
};
